  ...
  <div class="container">
   <div class="row">
    ...
    <div class="row d-flex mx-2">
      <input class="form-control  " type="text" v-model="newTaskTitle" @drop="editItem"  placeholder="Thêm việc cần làm" required>
      <button class="btn btn-success mt-2 h3 " @click=" addTask()">Add Task</button>
      <button class="btn btn-success mt-2 h3 " v-show="isEditItem" @click=" SaveItem()">Save Task</button>
    </div>
  </div>
...
export default {
  ...
  if (this.draggedTaskIndex !== null && this.tasks.findIndex(task => task.id === this.draggedTask.id) != -1) {
    ...
    this.tasks.splice(this.draggedTaskIndex, 1);  //
    this.tasks.splice(targetIndex, 0, {...draggedItem});    //copy từ mảng mới phông phụ thuộc vào mảng gốc
    // Đặt draggedItemIndex về null để kết thúc quá trình kéo thả
    this.draggedTaskIndex = null;
  }else{
    this.tasks.splice(targetIndex + 1, 0, {... this.tasksComp[this.draggedTaskIndex]});
    this.tasksComp.splice(this.draggedTaskIndex, 1);  
  ...
  if (this.draggedTaskIndex !== null && this.tasksComp.findIndex(task => task.id === this.draggedTask.id) != -1 ) {
    ...
    // Đổi chỗ hai item trong mảng
    this.tasksComp.splice(this.draggedTaskIndex, 1);
    this.tasksComp.splice(targetIndex, 0, {...draggedItem});  //copy từ mảng mới phông phụ thuộc vào mảng gốc
    // Đặt draggedItemIndex về null để kết thúc quá trình kéo thả
    this.draggedTaskIndex = null;
  }else{
    this.tasksComp.splice(targetIndex + 1, 0, {... this.tasks[this.draggedTaskIndex]});
    this.tasks.splice(this.draggedTaskIndex, 1);  

  }},
  editItem(){
    // if (this.tasks[this.draggedTaskIndex]) {
      this.newTaskTitle = this.tasks[this.draggedTaskIndex].title;
      this.isEditItem = true;
      this.editIndex = this.draggedTaskIndex;
      console.log(this.editIndex)
    // }

  },
  SaveItem(){
    this.tasks[this.editIndex].title = this.newTaskTitle;
    this.isEditItem = false;
    this.newTaskTitle = '';
