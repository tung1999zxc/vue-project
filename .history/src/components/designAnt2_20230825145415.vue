...
export default {
  ...
  if (this.draggedTaskIndex !== null) {
    ...
    const draggedItem = this.items[this.this.draggedTaskIndex];

    // Lưu item tại vị trí thả vào biến
    const targetItem = this.items[targetIndex];

    // Đổi chỗ hai item trong mảng
    this.items.splice(this.this.draggedTaskIndex, 1, targetItem);
    this.items.splice(targetIndex, 1, {...draggedItem});  //copy từ mảng mới phông phụ thuộc vào mảng gốc

    // Đặt draggedItemIndex về null để kết thúc quá trình kéo thả
    this.this.draggedTaskIndex = null;
  }
}
