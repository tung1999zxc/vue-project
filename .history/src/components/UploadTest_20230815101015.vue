<template>
  <div class="student-list">
    ...
    <table>
      ...
      <tbody>
        <tr v-for="(student,index) in students" :key="index">
          ...
          <td>{{ student.age }}</td>
          <td>
            <button @click="editStudent(student,index)">Edit</button>
    ...
    <div class="form" >
      <input v-model="newStudentName" placeholder="Enter student name" required/>
      <input v-model="newStudentAge" placeholder="Enter student name" required/>
      <button @click="addStudent">Add Student</button>
    </div>
    <div v-if="editingStudent">
      <h3>Edit Student</h3>
      <input v-model="editingStudent.name" />
      <input v-model="editingStudent.age" />
...
const edittingIndex = ref();

const newStudentName = ref('');
const newStudentAge = ref<any>();
const editingStudent = ref<Student | null>(null);

const addStudent = () => {
  if (newStudentName.value.trim() !== '' & newStudentAge.value.trim() !== '') {
    students.value.push({ id: students.value.length + 1, name: newStudentName.value, age: newStudentAge.value });
    newStudentName.value = '';
    newStudentAge.value = '';
  }
};

const editStudent = (student: Student, index) => {
  edittingIndex.value = index;
  editingStudent.value = { ...student };
};

const saveEdit = () => {
      students.value[edittingIndex.value] = editingStudent.value;
