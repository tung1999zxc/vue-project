...
const handleDelete = (index: number) => {
  ...
  const handleCompeleted = (todo: Todo , index: number ) => {
    ...
    const startTimeDate = parse(startTime, dateFormat, new Date());
const endTimeDate = parse(endTime, dateFormat, new Date());

    alert (todo.id);
    alert (todo.timeEnd);
    const minutes= differenceInMinutes(new Date (todo.id), new Date(todo.timeEnd)) % 60;
    alert(minutes)
