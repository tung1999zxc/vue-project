   ...
   <div class="row justify-content-around">
    ...
    <div class="col-3">
      <table class="table table-striped    ">
        ...
        <tr v-for="todo in todosCompeleted" :key="todo.id">
          <td><p>{{ todo.id }}</p></td>
          <td class=""><p class="text-danger">{{ todo.title }}</p></td>
          <td> <button @click="handleDeleteComp(todo.id)">Delete</button></td>
        </tr>
        <td ></td>
      </table>
    </div>
...
const handleDelete = (index: number) => {
  ...
  const handleDeleteComp = (todo.id: number) => {
    const index = todosCompeleted.value.findIndex(todo=>todo.id ===todo.Id);
    if (index !== -1){
  ...
  const handleCompeleted = (todo: Todo , index: number ) => {
    ...
    console.log(todosCompeleted.value)
    // todos.value.splice(index,1);
    saveDataToLacal();
