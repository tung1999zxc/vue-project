...
// Sử dụng computed để truy cập các getters
const isAuthenticated = computed(() => store.getters['auth/isAuthenticated']);
const userinfo = computed(() => store.getters['auth/userinfo']);

// Gọi các mutations và actions từ store
const { login, logout } = store.actions['auth'];
const { SET_AUTHENTICATED } = store.mutations['auth'];
    // handleLogin() {
    //   this.login(); // Call the login action to set isAuthenticated to true
    // },
    // handleLogout() {
    //   logout(); // Call the logout action to set isAuthenticated to false
    // };
