  ...
  <div>
    ...
    <ul>
      <li v-for="(name,index) in names" :key="index">{{ name }}
        ...
        <button @click="handleEdit(index,name)">sửa</button>
      </li>
    </ul>
...
const handleEdit = (index,name) => {
  ...
  alert(edittingItem.value)
  newName.value = name;
  
}
const handleUpdate = () => {
  names.value[edittingItem.value] = newName.value;
  //  const updateName = newName.value;
  //  names.value.splice(index,1,updateName);
